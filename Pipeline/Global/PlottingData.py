...
class PlottingData(Plotting):
    ...
    def plot_hfa_severity(
        ...
        with cls._style_context():
            ...
            sns.barplot(
                ...
                # --- [核心改进] 临床警示渐变色板 ---
                # 从浅黄(正常)平滑过渡到深红(极重度)，视觉直接映射疾病严重性
                palette='YlOrRd',
    ...
    def plot_feature_distributions(
        ...
        with cls._style_context():
            ...
            if is_final_record:
                safe_name = title.replace(":", "").replace(" ", "_").replace("(", "").replace(")", "")
                cls._save_figure(fig=fig, prefix="EDA", experiment_name=safe_name, fitness_metric="")
    ...
    def plot_target_correlation(
        ...
        # --- [核心修正 1] 排序倒转 ---
        # 改为 ascending=True。配合底部的 invert_yaxis()，
        # 负值最大（最靠左）的特征将被推到图表的最上方（左上角）。
        ...
        with cls._style_context():
            ...
            # [视觉瘦身] 将柱子的高度 (height) 从 0.7 降至 0.55，增加柱子之间的留白
